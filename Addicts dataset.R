...
addicts[1:5,1:6]
 ...
 # 1 - Creating a survival object

library(survival)
Surv(addicts$survt,addicts$status==1)

 # 2 - Estimating survival functions (unadjusted) and comparing them across strata
...
# 3 - Assessing the PH assumption using graphical approaches
...
legend("bottomright",c("Clinic 1","Clinic 2"),
 ...
 #  4 Running a COX PH model

Y=Surv(addicts$survt,addicts$status==1)

coxph(Y~prison + dose + clinic,data=addicts)

 # Detailed COX PH model

summary(coxph(Y~prison+dose+clinic,data=addicts))

 # Handling ties (i.e. more than one event in a defined time interval) - several methods
...
coxph(Y~prison + dose + clinic,data=addicts, method="exact")
 ...
 # - mod 1 - the reduced model 
 # - mod 2 - the full model
...
mod2
 ...
 # Utilising information from mod 1 and mod 2

names(mod2)

 # Call up the log-likelihood function
...
# calculate the difference in -2 Log-likelihood (deviance) of the two models

(-2)*(mod1$loglik[2]-mod2$loglik[2])

 # Determine the significance of the difference between the models using Chi-square statistic

LRT=(-2)*(mod1$loglik[2]-mod2$loglik[2])
Pvalue = 1 - pchisq(LRT,2)
Pvalue

 # Determine the significance of the difference between models using a self-defined function.
...
coxph(Y~ prison + dose + strata(clinic),data = addicts)
 ...
 # Including interaction term in a stratified Cox model

coxph(Y~prison + dose + clinic:prison + clinic:dose + strata(clinic),data=addicts)

 # Estimating hazard ratio for PRISON=1 vs PRISON=0 for CLINIC=2
...
cox.zph(mod1,transform=rank)
 ...
 # plot of schoenfeld residuals against each individual's failure time.
...
plot(survfit(mod1, newdata=pattern1),
 ...
 # stratified Cox adjusted survival curves
...
par(new=F)
 ...
 # 8 - RUNNING AN EXTENDED COX MODEL
